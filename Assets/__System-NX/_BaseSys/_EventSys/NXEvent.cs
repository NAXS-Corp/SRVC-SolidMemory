// Source: https://assetstore.unity.com/packages/tools/integration/easy-event-manager-147887
// http://tigerforge.altervista.org/easyeventmanager/?doing_wp_cron=1604989118.4494929313659667968750
using Ludiq;
using UnityEngine;
...
namespace NAXS.Event{
    ...
    public class NXEvent {
        ...
        {
            #if UNITY_EDITOR
            Debug.Log("NXEvent>> EmitEvent:: "+eventName);
            #endif
        ...
        public static void EmitEvent(string eventName, object sender)
        {
            ...
            if (NXEvent.sender.ContainsKey(eventName)) NXEvent.sender[eventName] = sender; else NXEvent.sender.Add(eventName, sender);
    ...
    public class EventsGroup
    {
        ...
        public void StartListening()
        {
            ...
            {
                NXEvent.StartListening(g.name, g.callBack);
            }
        }
        ...
        public void StopListening(string eventName = "")
        {
            if (eventName == "")
            {
                foreach (SEvent g in group)
                {
                    NXEvent.StopListening(g.name, g.callBack);
                }
            }
            else
            {
                ...
                foreach (SEvent g in group)
                {
                    if (g.name != eventName) newGroup.Add(g); else NXEvent.StopListening(g.name, g.callBack);
                }
                group = newGroup;
            }
